...
W=tf.Variable(tf.random_normal([2*n_hidden,1]))

attn_weights = tf.squeeze(tf.matmul(output, W), 2) # attn_weights : [batch_size, n_step]
...
with tf.Session() as sess:
    ...
    print(attention)
    print('****')
    print(sess.run(W,feed_dict={X: input_batch, Y: target_batch}))
    # Test
